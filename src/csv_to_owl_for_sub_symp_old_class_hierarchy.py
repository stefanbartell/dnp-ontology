...
groups = list(set(kb_dnp.loc[:,"GROUP"]))

substance_groups = []
symptom_groups = []
for group in groups:
    if "substance" in str(kb_dnp.loc[kb_dnp['GROUP'] == group, 'SUBSTANCE/SYMPTOM']):
        substance_groups += [group]
    if "symptom" in str(kb_dnp.loc[kb_dnp['GROUP'] == group, 'SUBSTANCE/SYMPTOM']):
...
# create the chemical substance class
class symptom(generically_dependent_continuant):
    namespace = onto

# create the chemical substance label class
class symptom_label(generically_dependent_continuant):
    namespace = onto
...
groups_original = sorted(list(set(kb_dnp.loc[:,"GROUP"])))
...
for group in substance_groups_original:
...
for group in symptom_groups_original:
    standardized_mapping_terms = sorted(list(set(kb_dnp.loc[kb_dnp['GROUP'] == group, 'STANDARDIZED MAPPING TERM'])))
    for term1 in standardized_mapping_terms:
        name1 = "onto."+lowerplusgroup(group)
        with onto:
        ...
        for label in standardized_textual_labels:
            name2 = "onto."+lowerplussmt(term1)
            with onto:
                NewClass = types.new_class(lowerplusstl(label), (onto.symptom_label,))
            textual_mentions = sorted(list(set(kb_dnp.loc[kb_dnp['STANDARDIZED TEXTUAL LABEL'] == label, 'TEXTUAL MENTION'])))
            for textual_mention in textual_mentions:
                name3 = "onto."+lowerplusstl(label)
                with onto:
                    NewClass = types.new_class(lowerplustm(textual_mention), (onto.symptom_label,))
...
onto.symptom.is_instance_of = [generically_dependent_continuant]

onto.symptom_label.is_instance_of = [generically_dependent_continuant]
